...
import stockService, { Stock } from '@/core/services/stockService';
...
export default function Registration() {
  ...
  // ==========================================
  // FETCH REGISTRATIONS, PACKAGES, KLOTER
  // ==========================================
  ...
  const [filterPackage, setFilterPackage] =
    useState('');
  ...
  const [formData, setFormData] =
    useState<Partial<Pilgrim>>({});

  const [stocks, setStocks] =
    useState<Stock[]>([]);

  const [modifiedDates, setModifiedDates] =
    useState<{
      [key: string]: boolean;
    }>({});
  ...
  const confirmDelete = async () => {
    try {
      ...
      toast(
        'Data pendaftaran berhasil dihapus (dibatalkan).',
        'success'
      );
    } catch (err: any) {
      toast(
        'Gagal menghapus/membatalkan pendaftaran dari server.',
        'error'
      );
    } finally {
      setFetchingRegistrations(false);
  ...
  const openAddModal = () => {
    const newFormId = `FRM-${Math.floor(
      100 + Math.random() * 900
    )}`;
  ...
  const savePilgrim = async () => {
    ...
    const payNotes =
      ...
      (payOption === 'DP'
        ? 'DP Uang Muka Pendaftaran'
        : payOption ===
          'Bayar Lunas'
          ? 'Pembayaran Lunas Pendaftaran'
    ...
    if (editingPilgrim) {
      try {
        ...
        toast(
          'Data pendaftar dan status pembayaran berhasil diperbarui.',
          'success'
        );
        ...
        toast(
          err.message ||
            'Gagal memperbarui pendaftaran ke server.',
          'error'
        );
      } finally {
        setFetchingRegistrations(false);
      ...
      try {
        ...
        toast(
          'Pendaftaran jamaah baru berhasil disimpan di server.',
          'success'
        );
        ...
        toast(
          err.message ||
            'Gagal menyimpan pendaftaran ke server.',
          'error'
        );
      } finally {
        setFetchingRegistrations(false);
  ...
  const getEquipment = (
    ...
    return formData.equipments?.find(
      (equipment) =>
        equipment.equipment_name === name
    );
  };

  const getPilgrimEquipment = (
    pilgrim: Pilgrim,
    name: string
  ) => {
    return pilgrim.equipments?.find(
      (equipment) =>
        equipment.equipment_name === name
    );
  };
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(
      exportData,
      'Data_Pendaftaran_Jamaah',
      'Laporan Pendaftaran Jamaah & Administrasi - DNA Tour'
    );
  };
        ...
        >
          <CardContent className="p-5 flex flex-col justify-between h-full gap-3">
            ...
            <div className="flex items-center gap-1.5 text-xs font-medium text-emerald-700">
              ...
              <span>
                Paspor, KTP, Vaksin & Foto Terpenuhi
              </span>
            </div>
          </CardContent>
        </Card>
          ...
          <Table className="min-w-[1050px] w-full">
            <TableHeader className="bg-gray-50/80">
              <TableRow className="hover:bg-transparent border-b-gray-200">
                ...
                <TableHead className="text-[11px] font-bold text-gray-500 uppercase tracking-wider h-11 whitespace-nowrap min-w-[110px]">
                  ID JAMAAH
                </TableHead>

                <TableHead className="text-[11px] font-bold text-gray-500 uppercase tracking-wider h-11 whitespace-nowrap min-w-[220px]">
                  Pendaftar
                </TableHead>

                <TableHead className="text-[11px] font-bold text-gray-500 uppercase tracking-wider h-11 whitespace-nowrap min-w-[160px]">
                  Paket
                </TableHead>

                <TableHead className="text-[11px] font-bold text-gray-500 uppercase tracking-wider h-11 whitespace-nowrap min-w-[140px]">
                  Status Dokumen
                </TableHead>

                <TableHead className="text-[11px] font-bold text-gray-500 uppercase tracking-wider h-11 whitespace-nowrap min-w-[180px]">
                  Status Pembayaran
                </TableHead>

                <TableHead className="text-right pr-6 text-[11px] font-bold text-gray-500 uppercase tracking-wider h-11 whitespace-nowrap min-w-[110px]">
                  Aksi
                </TableHead>
              </TableRow>
            </TableHeader>
              ...
              {paginatedData.map(
                (pilgrim) => {
                  ...
                  return (
                    ...
                    >
                      <TableCell
                        className="pl-4 py-4 whitespace-nowrap"
                        onClick={(e) =>
                          e.stopPropagation()
                        }
                      >
                  ...
                  <div>
                    ...
                    <div className="bg-white border border-[#cbd5e1] rounded-3xl overflow-hidden divide-y divide-[#e2e8f0] shadow-2xs">
                      <div className="flex items-center justify-between py-3.5 sm:py-4 px-5 sm:px-6">
                        ...
                        <span className="font-bold text-gray-900 text-sm text-right">
                          {formatIndoDate(
                            activePilgrim?.registrationDate
                          )}
                        </span>
                      </div>
                  ...
                  <div>
                    ...
                    <div className="bg-white border border-[#cbd5e1] rounded-3xl overflow-hidden divide-y divide-[#e2e8f0] shadow-2xs">
                      {[
                        {
                          name: 'Koper Besar',
                          label: 'Koper Besar 24"'
                        },
                        {
                          name: 'Koper Kabin',
                          label: 'Koper Kabin 20"'
                        },
                        {
                          name: 'Seragam Batik',
                          label: 'Seragam Batik'
                        ...
                        {
                          name: 'Kerudung Merah',
                          label: 'Kerudung Merah'
                        },
                        {
                          name: 'Kerudung Putih',
                          label: 'Kerudung Putih'
                        },
                        {
                          name: 'Tas Selempang',
                          label: 'Tas Selempang'
                        ...
                        (item) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span
                                className={`font-bold text-sm text-right flex items-center gap-1.5 ${
                                  isChecked
                                    ? 'text-[#00a859]'
                                    : 'text-gray-400'
                                }`}
                  ...
                  <div>
                    ...
                    <div className="bg-white border border-[#cbd5e1] rounded-3xl overflow-hidden divide-y divide-[#e2e8f0] shadow-2xs">
                      ...
                      <div className="flex items-center justify-between py-3.5 sm:py-4 px-5 sm:px-6">
                        ...
                        <span className="font-bold text-gray-900 text-sm text-right font-mono">
                          {activePilgrim?.totalAmount
                            ? `Rp ${activePilgrim.totalAmount.toLocaleString(
                                'id-ID'
                              )}`
                            : 'Rp 0'}
                        </span>
                      ...
                      <div className="flex items-center justify-between py-3.5 sm:py-4 px-5 sm:px-6">
                        <div className="flex items-center gap-3.5 text-gray-700 text-sm font-medium">
                          <CreditCard className="w-4 h-4 text-[#782820] shrink-0" />
                          <span>
                            Nominal Telah Dibayar
                          </span>
                        </div>

                        <span className="font-bold text-[#00a859] text-sm text-right font-mono">
                          {activePilgrim?.paidAmount
                            ? `Rp ${activePilgrim.paidAmount.toLocaleString(
                                'id-ID'
                              )}`
                            : 'Rp 0'}
                        </span>
            ...
            <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-12 gap-1.5 sm:gap-4 items-center">
                    ...
                    <div className="sm:col-span-8">
                      <Input
                        ...
                        placeholder="Cth. REG-1001 / PL-88210"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-12 gap-1.5 sm:gap-4 items-center">
                    ...
                    <div className="sm:col-span-8">
                      <Input
                        ...
                        className={`h-12 sm:h-13 rounded-2xl border-gray-300 bg-white text-base ${
                          modifiedDates.registrationDate &&
                          formData.registrationDate
                            ? 'font-bold text-gray-900'
                            : 'font-normal text-gray-400'
              ...
              <div className="bg-white border border-[#cbd5e1] rounded-3xl p-5 sm:p-7 shadow-2xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    return (
                      <div
                        key={item.name}
                        className="flex items-center gap-4 p-4 border border-gray-200 rounded-2xl bg-white shadow-2xs cursor-pointer hover:border-[#00a859] transition-colors"
                      >
                        <Checkbox
                          checked={
                            isChecked
                ...
                <div className="mt-4 p-4 border border-gray-200 rounded-2xl bg-gray-50">
                  ...
                  <select
                    ...
                    className={`h-12 w-full sm:w-1/2 rounded-xl border border-gray-300 bg-white px-4 text-sm ${
                      getEquipment(
                        'Sabuk'
                      )?.size
                        ? 'font-bold text-gray-900'
              ...
              <div className="bg-white border border-gray-200 rounded-3xl p-6 sm:p-8 shadow-sm">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-6 border-b border-gray-100 pb-5">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-xl bg-[#00a859] text-white flex items-center justify-center text-sm font-bold shadow-sm">
                      4
                    </div>
                  ...
                  <p className="text-sm text-gray-500 font-medium">
                    Pilih status pembayaran awal pendaftaran
                  </p>
                </div>
                  ...
                  <div className="border border-gray-200 rounded-2xl p-5 sm:p-6 bg-white shadow-sm">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
                      ...
                      <span className="text-sm text-gray-500 font-medium">
                        Otomatis mencatat transaksi di Keuangan
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-6 sm:gap-y-7 mb-6">
                        ...
                        <Input
                          ...
                          className={`h-12 rounded-xl text-lg ${
                            formData.totalAmount
                              ? 'font-bold text-gray-900'
                              : 'font-normal text-gray-400'
                          } border-gray-200 focus:border-[#00a859] focus:ring-[#00a859] placeholder:font-normal placeholder:text-gray-400`}
                        />
                      </div>
                      ...
                      <div className="flex flex-col gap-2.5 sm:gap-3">
                        ...
                        <Input
                          ...
                          className={`h-12 rounded-xl text-lg ${
                            formData.paidAmount
                              ? 'font-bold text-gray-900 border-[#00a859] bg-[#f0fdf4]'
                              : 'font-normal text-gray-400 border-gray-200 focus:border-[#00a859]'
                          } border-2 focus:ring-[#00a859] placeholder:font-normal placeholder:text-gray-400`}
                        />
                      </div>
                      ...
                      <div className="flex flex-col gap-2.5 sm:gap-3">
                        ...
                        <div className="relative">
                          <select
                            ...
                            className={`w-full h-12 rounded-xl border border-gray-200 bg-white px-4 text-sm ${
                              (formData as any)
                                .paymentMethod
                                ? 'font-bold text-gray-900'
                                : 'font-normal text-gray-400'
                            } focus:border-[#00a859] focus:ring-1 focus:ring-[#00a859] appearance-none cursor-pointer`}
                          >
